...
# 在逐条核对，而不是一个人在给意见。
#
# 窗口一路收下来：「五到六百」→「五到五百五」→ 现在的「三到四百」。每收一档，
# 能装下的点就少一个，颗粒度是被篇幅逼下来的，靠规则劝说是劝不动的。
#
# 收到三到四百这一档是按一批已定稿的理由回推的：五百字那一档写出来，决定胜负的
# 缺陷讲完之后总还剩余量，于是内部机制的分步推演、次要缺陷、过程叙事就都被填了
# 进去；三到四百只装得下「做错了什么、导致什么后果、对方哪里做得好」这三件事，
# 剩下的必须舍掉。
#
# 上限和目标上限分开：目标是 400，真正会告警的是超过 500。中间这一段留给那些
# 确实需要同时交代两侧覆盖面的题，硬卡在 400 会把判断依据挤掉。
REASON_TARGET_MIN = 300
REASON_TARGET_MAX = 400
REASON_SOFT_MAX_CHARS = 500
...
一、篇幅与取舍
1. 正文写 {REASON_TARGET_MIN} 到 {REASON_TARGET_MAX} 字，{REASON_TARGET_MAX} 字出头也可以，
   无论如何不要超过 {REASON_SOFT_MAX_CHARS} 字。这是给人看的评审意见，不是核对清单。
