...
# 在逐条核对，而不是一个人在给意见。五百字出头够把一两个决定胜负的点讲透。
#
# 窗口从「五到六百」收到「五到五百五」：六百字那一档写出来仍然是每个点都要交代
# 一句，收到五百五就只装得下一两个点，剩下的必须舍掉，颗粒度是被篇幅逼下来的。
REASON_TARGET_MIN = 500
REASON_TARGET_MAX = 550
REASON_SOFT_MAX_CHARS = 570
...
# 「这么看下来 B 更好」。「判」字自带机械和居高临下的味道，前面写得再自然，收在
...
一、篇幅与取舍
1. 正文写 {REASON_TARGET_MIN} 到 {REASON_TARGET_MAX} 字，能落在 {REASON_TARGET_MIN} 字出头最好，
   不要超过 {REASON_TARGET_MAX} 字。这是给人看的评审意见，不是核对清单。
...
5. 开头别铺得太细。一个人看两份产物，能记住的就是一两件事，不会把两边的实现
...
11. 结论句换着写，也不要用「判」字。「判 A 更好」「判 B 更好」是裁判在宣判，人给
    意见不会这么说，写成「所以 A 更好」「这么看下来 B 更好」就行。
...
五、必须写到的内容
19. A 和 B 分别写，各自好在哪、差在哪，不要只写「A 比 B 好」就收尾。
20. 产物和过程都要写。过程问题要说清是在哪个环节出的、模型当时具体做了什么、
    由此造成了什么后果；产物问题要指到具体文件名、函数名、报错原文，或者哪一条
    需求没实现。
21. 提名字是为了让人能找到那个地方，不是为了把材料抄全。说清楚哪个文件里的哪个
    函数出了什么事就够了，一句话里不要连着报三四个名字，也不要每提一件事都配一个
    路径——真人写意见时记得住的就那么几个。
22. 要写出判断依据：你更看重哪几点、为什么这几点压过了另一侧的长处。两次跑通常
    各有优劣，要体现权衡，不要给没有来由的结论。
23. 结论选 Same 时同样写足，写清哪些点确实等价、哪些点各有优劣相互抵消。
    一句话的 Same 会被直接打回。
24. 主观评价后面要跟客观依据。说「实现得更完整」就要说出是哪个功能点、在哪个文件里。

六、绝对不能出现的东西（这几类会被判成机器写的）
25. 不写步数：「第 38 步」「第 19、20 步」「步骤 12」「第 3 轮」一次都不要出现。
26. 不写只有程序数得出来的量：工具调用次数、总步数、交互轮次、新增删除了多少行、
    改了多少个文件、耗时多少秒或多少分钟。
27. 不写文件行号区间：可以写 lib/dumper.js，不要写 lib/dumper.js:120-136。
28. 不要把终端输出原样搬进来，例如「# tests 32 # pass 32 # fail 0」「43 过 1 败」。
29. 不要一口气罗列五个以上的测试名或函数名。
30. 控制数字密度：整段里精确数字不要超过每百字两个。业务本身要求的数字照写，
    但不要靠数字堆出对比。
31. 不用固定分栏：不要【产物】【过程】这类栏目，不要「好：」「不好：」，
    不要 1. 2. 3. 的编号。
32. 不用表情符号，不用 markdown（标题井号、列表符号、加粗星号、反引号都不要），
    平台的理由框是纯文本，这些记号会原样显示出来。
33. 不写自己是什么：不要出现「作为 AI」「作为大语言模型」这类话，也不要
    「以下是我的分析」「希望对你有帮助」这种对话腔。

七、容易写空的地方
34. 少用「各有优劣」「难分高下」「看不出差别」「表现良好」「基本可用」「更好一些」
    这类词。用了就必须紧跟具体的事实，否则整段会被判成没有实质内容。
35. 过渡词不必刻意躲，但不要拿「综上所述」「总的来说」「值得注意的是」当段落骨架
    反复用，同一段里出现两个以上就会被判成套话。

八、关于「正式」的分寸
36. 不必为了像人而刻意堆口语、刻意每句都带「我」，写得专业不扣分。但也别端着写：
    用日常会说的词，句子短一点，能说「跑出来的结果不对」就不要说「运行时行为与
    预期存在偏差」。你是在给同事讲这两份产物差在哪，不是在交一份评估报告。
37. 文件名、函数名、方法名、命令、报错原文都可以照写，这是定位问题的正常方式，
    只要不带行号。路径太长时可以只写文件名或者说清是哪个目录，不必背全路径。

九、不作为判断依据的因素
38. 只写模型自身能力造成的差异。推理快慢、网络波动、网关超时、请求失败、
    模型没报错但戛然而止，这几类都受部署和排队影响，不反映能力，不写进理由。
    如果某一侧确实出现了这类情况，只在 remark 里写一句说明。
39. 提到文件只写仓库内的相对路径，例如 lib/rules_inline.mjs。不要出现任何绝对
    路径或本机目录名——这段理由会原样交给评审方，写进去等于把本地目录结构一起交出去。
40. 不写你是怎么核验的：不写有没有装依赖、能不能联网、跑没跑起来，也不要出现
