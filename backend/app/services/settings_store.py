...
SPECS: tuple[Spec, ...] = (
    ...
    # 难度筛选分一前一后两道关，都在拦难度不够的题，省的东西不一样。
    # 跑之前看题面要动几个模块（scope 模块），拦下来省的是两个容器的机器时间；
    # 跑之后看两侧实际用了多少步、多少分钟（difficulty 模块），拦下来省的是那次分析额度。
    # 两处的默认值都必须和各自模块里的 *_DEFAULT 一字不差，否则设置页显示一套、实际按另一套判。
    Spec("difficulty.min_modules", "开跑前要求题面至少跨几个模块", "难度筛选",
         default="2", kind="number",
         help="领取时让模型读一遍题面，数出这道题非改不可的模块有几个，不够就在门禁上拦下。"
              "改动面窄的题两边十几步都能做完，比出来的只有措辞差异，而跑一道要两个容器"
              "各两个多小时。判错了可以在题目详情页放行，或者直接强制启动。填 0 或 1 等于关掉"),
    Spec("difficulty.probe", "先跑一侧探路，不行就不跑另一侧", "难度筛选", default="1", kind="bool",
         help="第二侧在队列里排到队尾，别的题的首侧先走。等它排上来时，先跑完那一侧"
              "通常已经有结论了：又快又轻就整题废弃，那一侧一次都不起，省下实打实的"
              "容器时间。槽位空着的时候第二侧照样出闸，不会为了探路让机器闲着"),
    Spec("difficulty.probe_steps", "探路：先跑完那一侧低于这个步数才算轻", "难度筛选",
         default="40", kind="number",
         help="要和下面的用时同时不达标才废弃。手上只有一侧的信息，所以比两侧都跑完时"
    ...
    Spec("difficulty.screen", "跑完先筛一遍难度", "难度筛选", default="1", kind="bool",
         help="两侧都跑得太轻的题在开 GSB 分析之前就废弃掉，省下那次分析与两道质检的额度。"
              ...
              "关掉则照旧全部送去分析"),
    Spec("difficulty.min_minutes", "两侧都低于这个用时就废弃（分钟）", "难度筛选",
         default="30", kind="number",
         help="容器用时，也就是列表行里显示的那个时长。只有两侧都低于它才废弃，"
              "一侧快一侧慢说明题确实难住了一边"),
    Spec("difficulty.min_steps", "两侧都低于这个步数就废弃", "难度筛选", default="40", kind="number",
         help="工具调用步数，也就是列表行里那个「N 步」。同样是两侧都低于才废弃"),
    Spec("difficulty.low_steps", "步数偏低线", "难度筛选", default="30", kind="number",
         help="一侧低于这条线时，另一侧必须确实吃力（见下一项）才算这道题有可比的东西，"
              "否则那点落差只是风格差异"),
    Spec("difficulty.low_peer_steps", "一侧低于偏低线时，另一侧至少要有的步数", "难度筛选",
