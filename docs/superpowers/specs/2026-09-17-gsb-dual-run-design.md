...
solo-prompt 的 SOP 已经改成建仓库时一并建好 A、B 两个分支，后端不再补建分支，只在门禁里
校验。若将来出现不合规的仓库，走人工修，不做自动修复。

## 3. 数据模型

清库重来，不做迁移。`data/solo-cli.db`、`data/exports/` 与 `prompt.md` 全部清空，
旧的 10 道题不再使用，重新出一批符合 GSB 口径（困难/地狱、仓库带 A/B 分支）的题。
