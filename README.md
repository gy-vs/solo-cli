...
仓库里只有代码。`data/`（库、`secret.key`、导出轨迹、池的本地副本）和 `.env` 都不进版本库，两台设备各自持有一份，互不覆盖；跨设备共享的只有查重池那一个仓库。
...
**本项目放在哪里都行，除了 `CODER_ROOT` 里面。** 容器只挂 `workspace/<题号>/<侧>` 与 `sessions/<题号>/<侧>` 两棵子树，把本项目 clone 到其中任何一棵下面，题库、判定点和查重池就随题目一起交到模型手上了，而界面上看不出任何异常。出题页的「出题资料隔离」检查会挡住这种配法，它属于阻塞项。

出题规则不在这个仓库里，在 `~/.cursor/skills/solo-prompt`。**只在一台设备上出题**：另一台同步代码后只做领题、跑题、分析、上传，不点「设计题目」，也不用配查重池——池是给两边都出题的场景准备的，单边出题时那边填了反而会把同一批题以第二个设备标识再入池一次。
